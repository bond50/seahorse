...
export const preSignup = (req, res) => {
    ...
    User.findOne({email: email.toLowerCase()}, (err, user) => {
        ...
        const emailData = {
            ...
            to: email,
            subject: `Welcome to Seahorse Energy LTD`,
            html: `
            <p>Please use the following link to activate your account.The link expires after 10 minutes</p>
            <p>${clientUrl}/auth/account/activate/${token}</p>
           <br>
          
            <p>This email may contain sensetive information</p>
            <p>Please do not reply to this email coz no one will receive it</p>
            <p>${process.env.CLIENT_URL}</p>`
        }
        sgMail.send(emailData).then(sent => {
            return res.json({
                message: `Account activation link has been sent to ${email}.The link expires after 10 minutes.`
            });
        });
    })
}
