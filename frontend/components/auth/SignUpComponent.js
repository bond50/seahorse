...
const SignupComponent = () => {
    ...
    const {name, email, password, error, loading, message,password1, showForm} = values;
    ...
    const signupForm = () => {
        return (
            <form onSubmit={handleSubmit} className='row gy-3'>
                ...
                <div className="col-12">
                    <label htmlFor="password1" className="form-label">Repeat password</label>
                    <div className="input-group ">
                        <input
                            value={password1}
    ...
    return (
        <AuthWrapper register signupLabel={!message?"Enter your personal details to create account":"Check Email to complete Registration  process"}>
            {showLoading()}
            {showError()}
            {showMessage()}
            {showForm && signupForm()}
